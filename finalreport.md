...
We observe that our lock-free cuckoo hash table greatly outperforms Intel's `concurrent_hash_map` under this test scenario. This is because Intel's implementation uses reader-writer locks which have much higher overhead than our two-round querying procedure. We also note that our implementation outperforms a `C++11 unordered_map`, though only by a factor of 3x.
